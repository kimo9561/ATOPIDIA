...
<jsp:include page="../nav/navbootstrap.jsp"></jsp:include>
	...
	<br>
		...
		<!-- Page Content -->
		<div class="container">
		<h1 style="text-align: center;">유익한 정보</h1>
		<br> <br>
		...
		<div class="container">
			<div class="row">
				<c:forEach var="dtos" items="${pageDto.data}">
					<div class="col-6">
						<div class="card">
							<div class="card-header" id="review" style="text-align: center ">${dtos.info_Title}</div>
							<img class="card-img-top" src="img/${dtos.info_Image_Realname}" width="300px" height="300px">
							<div class="card-body">
								<p class="card-text">작성자:${dtos.info_Writer}</p>
								<a href="admininfocontroller.do?command=detail&info_No=${dtos.info_No}" class="button3" id="reviewbutton">리뷰 더보기 &rarr;</a>
							</div>
	...
	<div id="extra2" class="container">
		<input type="button" class="button3" value="글쓰기" style="float: right;"
			onclick="location.href='admininfocontroller.do?command=writeform'">
	</div>
	<br>
	<br>
	<br>
	...
	<ul class="pagination my justify-content-center mb-4">
 		<c:if test="${pageDto.startPage != 1}">
			<li><a class="page-link text-dark" href="admininfofocontroller.do?command=list&page=${pageDto.startPage-1}">◀</a></li>
 		</c:if>
		
		<c:forEach var="i" begin="${pageDto.startPage}" end="${pageDto.endPage}">
			<c:if test="${pageDto.page == i}"><p class="page-link text-dark">${i}</p></c:if>
			<c:if test="${pageDto.page != i}">
				<li><a class="page-link text-dark" href="admininfocontroller.do?command=list&page=${i}">${i}</a></li>
			</c:if>
		</c:forEach>

		<c:if test="${pageDto.endPage != pageDto.totalPage}">
			<li><a class="page-link text-dark" href="admininfocontroller.do?command=list&page=${pageDto.endPage+1}">▶</a></li>
		</c:if>
