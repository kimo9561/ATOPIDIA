...
public class UserInfoController extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      UserInfoBizImpl biz = new UserInfoBizImpl();
      DiaryBizImpl dbiz = new DiaryBizImpl();
      	...
      	if (command.equals("list")) {         
		       ...
		       try {
				...
				RequestDispatcher dis = request.getRequestDispatcher("user_board/userInfo_home.jsp");
			...
			UserInfoAnswerDto dto = new UserInfoAnswerDto(info_No, answer_writer, answer_content);
			
			int res = biz.insertAnswer(dto);
			
			if(res>0) {
				jsResponse("답글 작성 성공","userinfocontroller.do?command=detail&info_No="+info_No,response);
			}else {
				jsResponse("답글 작성 실패","userinfocontroller.do?command=detail&info_No="+info_No,response);
			}
         
      } else if (command.equals("writeform")) {
         response.sendRedirect("user_board/UserInfo_write.jsp");
         System.out.println("[" + command + "]");   
		...
		System.out.println(info_Image_Uploadpath);
         ...
         UserInfoDto userinfowrite = new UserInfoDto(member_No ,writer, title, content, info_Image_Name, info_Image_Realname, info_Image_Uploadpath);
         
         boolean res = biz.insertInfo(userinfowrite);
         
         if(res) {
            jsResponse("글 작성 성공", "userinfocontroller.do?command=list", response);
         } else {
            jsResponse("글 작성 실패", "userinfocontroller.do?command=boardwrite", response);
         }
    	 ...
    	 int info_No = Integer.parseInt(request.getParameter("info_No"));
         List<UserInfoAnswerDto> ans = biz.answerlist(info_No);
         boolean res = biz.addHits(info_No);
         UserInfoDto inf = biz.selectOne(info_No);
         
         request.setAttribute("list", inf);
         request.setAttribute("ans", ans);
         System.out.println(inf.toString());
         dispatch("user_board/UserInfo_view.jsp", request, response);
         ...
         if(res) {
            jsResponse("글 삭제 성공", "userinfocontroller.do?command=list", response);
         } else {
            jsResponse("글 삭제 실패", "userinfocontroller.do?command=list", response);
         }
      } else if(command.equals("updateInfoForm")) {
         int infono = Integer.parseInt(request.getParameter("info_No"));
         UserInfoDto one = biz.selectOne(infono);
         
         request.setAttribute("one", one);
         dispatch("user_board/userInfo_update.jsp", request, response);
         ...
         int info_No = Integer.parseInt(multi.getParameter("info_No"));
         int member_No = Integer.parseInt(multi.getParameter("member_No"));
         System.out.println(info_No);
         String title = multi.getParameter("info_Title");
         String text = multi.getParameter("info_Text");
         
         UserInfoDto dto = new UserInfoDto(info_No, member_No , title, text, info_Image_Name, info_Image_Realname, info_Image_Uploadpath);
         
         boolean res = biz.updateInfo(dto);
         
         if(res) {
            jsResponse("글 수정 성공", "userinfocontroller.do?command=list", response);
         }else {
            jsResponse("글 수정 실패", "userinfocontroller.do?command=updateInfoForm&info_No="+info_No, response);
         }
			...
			if(res) {
				jsResponse("답글 삭제 성공","userinfocontroller.do?command=detail&info_No="+info_No,response);
			}else {
				jsResponse("답글 삭제 실패","userinfocontroller.do?command=detail&info_No="+info_No,response);
			}
			...
			List<UserInfoAnswerDto> ans = biz.answerlist(info_No);
			UserInfoDto dto = biz.selectOne(info_No);
			UserInfoAnswerDto anw = biz.answerSelectOne(info_No, answer_No);
			...
			dispatch("user_board/UserUpdateInfoAnswer.jsp", request, response);
			...
			UserInfoAnswerDto dto = new UserInfoAnswerDto(info_No,answer_No,answer_Writer,updateContent);
			
			boolean res = biz.updateAnswer(dto);
			
			if(res) {
				jsResponse("답글 수정 성공","userinfocontroller.do?command=detail&info_No="+info_No,response);
			}else {
				jsResponse("답글 수정 실패","userinfocontroller.do?command=updateanswerform&info_No="+info_No+"&answer_No="+answer_No,response);
			}
